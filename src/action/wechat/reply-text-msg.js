...
module.exports = async msgData => {
  ...
  switch (content) {
    ...
    // 回复图片消息
    case '2':
      data = {msgType: 'image', mediaId: await upload('image', 'static/img.jpg')}
      break
    // 回复语音消息
    case '3':
      data = {msgType: 'voice', mediaId: await upload('voice', 'static/music.mp3')}
      break
    // 回复视频消息
    case '4':
      data = {msgType: 'video', mediaId: await upload('video', 'static/video.mp4'), title: '精彩的视频', description: '精彩的视频的描述'}
      break
    // 回复音乐消息
    case '5':
      data = {
        ...
        title: '胜似天籁',
        description: '美妙绝伦的音乐',
        musicURL: await upload('voice', 'static/music.mp3'),
        hQMusicUrl: await upload('voice', 'static/music.mp3'),
